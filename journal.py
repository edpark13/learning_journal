...
"""
SELECT_SINGLE_ENTRY_WTT = """
SELECT * FROM entries WHERE title=%s AND text=%s;
"""
...
#     return get_single_entry_wtt(request)

    # return HTTPFound(request.route_url('home')) # return id, title, created, text

@view_config(route_name='add', renderer='json')
def add_entry(request):
    if request.authenticated.authenticated_userid:
...
def get_single_entry_wtt(request):
    title = request.params.get('title', None)
    text = request.params.get('text', None)
    cursor = request.db.cursor()
    cursor.execute(SELECT_SINGLE_ENTRY_WTT, (title, text))
    keys = ('id', 'title', 'text', 'created')
